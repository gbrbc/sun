...
def makerecdeg(line_lonlat, dim2, k):

    if not isinstance(line_lonlat,LineString):
        Deb(line_lonlat.geom_type.unique())
...
#ORIG    target_crs = CRS("EPSG:3857")  # Web Mercator (projected in meters) - Choose a suitable UTM zone for better accuracy if working with specific regions
    ...
    offset_line_projected = line_lonlat.offset_curve(offset_distance_meters, join_style=1, mitre_limit=5,quad_segs=1) # join_style=1 for mitered corners
##try    offset_line_projected = line_projected.offset_curve(offset_distance_meters, join_style=1, mitre_limit=5) # join_style=1 for mitered corners
    ...
    offset_line_lonlat=    offset_line_projected
...
#    end_point_offset = offset_line_lonlat.coords[-1]
    ...
    start_point_original = line_lonlat.geometry.iloc[0].coords[0]

    end_point_original = line_lonlat.geometry.iloc[0].coords[-1]
    start_point_offset = offset_line_lonlat.geometry.iloc[0].coords[0]
    end_point_offset = offset_line_lonlat.geometry.iloc[0].coords[-1]
    ...
    Deb(line_lonlat.geom_type.unique())
    Deb(offset_line_lonlat.geom_type.unique())
    Deb(type(connecting_line_start))
    Deb(type(connecting_line_end))

    Deb(line_lonlat)
    Deb(offset_line_lonlat)
    Deb(connecting_line_start)
    Deb(connecting_line_end)
    




#    Deb(connecting_line_start.geom_type.unique())
#    Deb(connecting_line_end.geom_type.unique())
